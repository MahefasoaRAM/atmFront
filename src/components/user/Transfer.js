...
function Transfer() {
  ...
  const submit = (e) => {
    ...
    setLoading(true)
    axios.post(`/api/transfer/${localStorage.getItem('id')}.${localStorage.getItem('account')}`, data).then(res => {
  ...
  const userlist = users.map(user => 
    (user.id !== parseInt(localStorage.getItem('id'))) ? (<option key={user.id} value={user.id}>{user.name}</option>) : null
    )
  ...
  return (
    <div className="container py-5">
      <div className='row justify-content-center'>
        ...
        <h1 className='text-center text-capitalize mb-4'>transfer</h1>
          <div className="card">
            <div className="card-body">
              ...
              <div className="form-group mb-3">
                ...
                <div className="form-group text-center">
                  <button type='submit' className='btn btn-primary text-capitalize'>transfer</button>
                </div>
              </form>
            </div>
          </div>
